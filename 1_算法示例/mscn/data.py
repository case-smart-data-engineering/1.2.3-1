...
def load_data(file_name, num_materialized_samples):
    ...
    # Load bitmaps
    with gzip.open(file_name + ".bitmaps.gz", 'rb') as f:
        for i in range(len(tables)):
            # 读取4个字节来表示当前查询中位图的数量
            four_bytes = f.read(4)
            if not four_bytes:
                print("Error while reading 'four_bytes' for query", i)
                exit(1)
            ...
            for j in range(num_bitmaps_curr_query):
                ...
                if not bitmap_bytes:
                    print("Error while reading 'bitmap_bytes' for query", i, "bitmap", j)
                    exit(1)

                # 将字节数据转换为numpy数组，并解包成位
                bitmaps[j] = np.unpackbits(np.frombuffer(bitmap_bytes, dtype=np.uint8))

            # 将当前查询的所有位图添加到列表中
            samples.append(bitmaps)

    print("Loaded compressed bitmaps")
    # num_bytes_per_bitmap = int((num_materialized_samples + 7) >> 3)
